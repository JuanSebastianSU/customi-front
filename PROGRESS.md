...
> como texto, etiquetas como JSON, `tipo` como texto; **los slots NO se cachean** (solo el conteo de piezas).
> `catalogo()`/`disfraces()` viejos intactos (RentaForm/VentaPos siguen server-side).
>
> **A6 detalle:** `MiEmpresaRepository` pasó su caché **de memoria a Room** (una sola fuente de verdad, y
> ahora sobrevive al reinicio); misma API pública (`mia/refrescar/editar/subir/limpiar`), cero cambios en
> Dashboard/Identidad. `PerfilRepository` estrena caché (antes no tenía): `observarPerfil()`/`refrescarPerfil()`,
> `PerfilViewModel` cache-first (sin parpadeo). Ambos son **una sola fila** (`id=0`) serializada a JSON. Logout
> limpia ambos (N1).
